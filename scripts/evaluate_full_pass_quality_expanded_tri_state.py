#!/usr/bin/env python
"""Evaluate tri-state decisions on expanded full-pass quality coverage."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--sequence-output", type=Path, default=Path("outputs/metrics/full_pass_quality_expanded_tri_state_sequence_eval.csv"))
    p.add_argument("--summary-output", type=Path, default=Path("outputs/metrics/full_pass_quality_expanded_tri_state_summary.csv"))
...
def decide(row: pd.Series, elev: float) -> str:
    passed = bool(row["accepted_per_pass_k_p01_p99"])
    if float(row["max_elevation_deg"]) < elev:
        ...
        summ.append(
            {
                ...
                "attack_accept_rate": accept / total if sample_type == "attack" and total else None,
                "legit_accept_rate": accept / total if sample_type == "legit" and total else None,
                "notes": "expanded tri-state full-pass only",
            }
        )
