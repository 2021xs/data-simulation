...
`CAUSAL_A_DOPPLER_RECONSTRUCTION_REPRODUCTION_VALIDATED`
...
| secondary_expected_rows                        |        200 |        200 | True     |
...
Independent spot-check 使用 `SHA256('R1_SPOTCHECK_V1|' + stable_row_identity)` 的 deterministic stratified selection，先覆盖 family × old-decision strata，再按 hash 补足 30 rows。结果：30/30 pass。

Verifier-v2 secondary cross-check：`PASS`，200/200 pass。

## 5. Required answers

1. Primary units：64/64 covered；64/64 通过全部 reproduction levels。
2. Primary rows：6280/6280 通过全部 reproduction levels；categorical requirements 见上表。
...
17. Earliest divergence layer：`NONE`。
18. Verifier-v2：`PASS`。
19. 30-row spot-check：30/30。
20. Reconstruction implementation 不改变 verifier science：支持。
21. R2 authorization：YES。
...
`NEXT STEP: CAUSAL_A_DOPPLER_CORE_RECONSTRUCTION_DESIGN_AND_FREEZE`
