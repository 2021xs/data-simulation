...
- target selection: `20-target table plus local TLE NORAD-sorted extension to 100 targets`
- target_count: `100`
- visible pass finder: 复用现有 Skyfield / SGP4 `find_pass` 逻辑
- visible mask: 使用当前 time_window 的 `min_elevation_deg`
- audit scan step: `30.0 s`；用于 availability 统计，elevation 定义与当前 verifier/pass finder 一致
...
- 7 days: high-quality targets = `100/100`, only-low = `0`, no-visible = `0`, median time-to-first-HQ = `7.47 h`, p90 = `16.00 h`
- 14 days: high-quality targets = `100/100`, only-low = `0`, no-visible = `0`, median time-to-first-HQ = `7.47 h`, p90 = `16.00 h`
- 30 days: high-quality targets = `100/100`, only-low = `0`, no-visible = `0`, median time-to-first-HQ = `7.47 h`, p90 = `16.00 h`

30 天窗口下：

- total targets = `100`
- visible target count = `100`
- targets with high-quality pass = `100`
- only-low targets = `0`，占比 `0.00%`
- mean time-to-first-high-quality = `7.98 h`
- median time-to-first-high-quality = `7.47 h`
- p90 time-to-first-high-quality = `16.00 h`
...
- 20-target baseline: `20/20` 有 high-quality pass，only-low = `0`，median time-to-first-HQ = `6.68 h`，p90 = `14.48 h`。
- 本轮 100-target: `100/100` 有 high-quality pass，only-low = `0`，median time-to-first-HQ = `7.47 h`，p90 = `16.00 h`。
